// mockData.js

export const mockUsers = [
  {
    id: "1",
    name: "Admin User",
    email: "admin@evwarranty.com",
    password: "password",
    role: "admin",
    phone: "0908050604",
    avatar: "/admin-interface.png",
    status: "active",
    createdAt: "2024-01-01T00:00:00Z",
  },
  {
    id: "2",
    name: "Nguyen Van A",
    email: "nguyenvana@evm.com",
    password: "password",
    role: "evm_staff",
    phone: "1900150xxx",
    avatar: "/diverse-office-staff.png",
    status: "active",
    createdAt: "2024-01-15T00:00:00Z",
  },
  {
    id: "3",
    name: "Tran Thi B",
    email: "tranthib@sc.com",
    password: "password",
    role: "sc_staff",
    serviceCenter: "SC Hanoi Central",
    phone: "1900150xxx",
    avatar: "/customer-service-interaction.png",
    status: "active",
    createdAt: "2024-02-01T00:00:00Z",
  },
  {
    id: "4",
    name: "Le Van C",
    email: "levanc@sc.com",
    password: "password",
    role: "sc_technician",
    serviceCenter: "SC Hanoi Central",
    phone: "1900150xxx",
    avatar: "/diverse-technician-team.png",
    status: "active",
    createdAt: "2024-02-15T00:00:00Z",
  },
];

export const mockWarrantyClaims = [
  {
    id: "1",
    claimNumber: "WC-2024-001",
    vehiclePlate: "30A-12345",
    vehicleModel: "VF 8",
    customerName: "Pham Van D",
    customerPhone: "0901234567",
    issueDescription: "Battery charging issue - not reaching full capacity",
    status: "in_progress",
    priority: "high",
    serviceCenter: "SC Hanoi Central",
    assignedTo: "Tran Thi B",
    createdAt: "2024-10-01T08:00:00Z",
    updatedAt: "2024-10-03T14:30:00Z",
    kilometers: 15000,
    estimatedCost: 5000000,
    parts: ["BAT-VF8-001"],
  },
  {
    id: "2",
    claimNumber: "WC-2024-002",
    vehiclePlate: "29B-67890",
    vehicleModel: "VF 9",
    customerName: "Hoang Thi E",
    customerPhone: "0912345678",
    issueDescription: "Infotainment system screen flickering",
    status: "pending",
    priority: "medium",
    serviceCenter: "SC HCMC District 1",
    createdAt: "2024-10-02T10:15:00Z",
    updatedAt: "2024-10-02T10:15:00Z",
    kilometers: 8500,
  },
  {
    id: "3",
    claimNumber: "WC-2024-003",
    vehiclePlate: "51F-11111",
    vehicleModel: "VF 7",
    customerName: "Le Thi F",
    customerPhone: "0923456789",
    issueDescription: "Air conditioning not cooling properly",
    status: "approved",
    priority: "medium",
    serviceCenter: "SC Da Nang",
    createdAt: "2024-10-03T14:20:00Z",
    updatedAt: "2024-10-03T16:45:00Z",
    kilometers: 12000,
    estimatedCost: 2500000,
  },
  {
    id: "4",
    claimNumber: "WC-2024-004",
    vehiclePlate: "92A-22222",
    vehicleModel: "VF 6",
    customerName: "Tran Van G",
    customerPhone: "0934567890",
    issueDescription: "Brake system making unusual noise",
    status: "completed",
    priority: "high",
    serviceCenter: "SC Can Tho",
    assignedTo: "Nguyen Van H",
    createdAt: "2024-09-28T09:00:00Z",
    updatedAt: "2024-10-01T17:30:00Z",
    kilometers: 18500,
    estimatedCost: 3200000,
  },
  {
    id: "5",
    claimNumber: "WC-2024-005",
    vehiclePlate: "43C-33333",
    vehicleModel: "VF 9",
    customerName: "Nguyen Thi H",
    customerPhone: "0945678901",
    issueDescription: "Steering wheel vibration at high speeds",
    status: "pending",
    priority: "urgent",
    serviceCenter: "SC Hanoi Central",
    createdAt: "2024-10-04T11:30:00Z",
    updatedAt: "2024-10-04T11:30:00Z",
    kilometers: 22000,
  },
];

export const mockParts = [
  {
    id: "1",
    serialNumber: "BAT-VF8-001",
    name: "Battery Module 87.7kWh",
    vehicleModels: ["VF 8", "VF 9"],
    cost: 45000000,
    warranty: 96,
    conditions: "Temperature controlled storage required",
    stock: 12,
    status: "available",
  },
  {
    id: "2",
    serialNumber: "INF-VF-001",
    name: 'Infotainment Display 15.6"',
    vehicleModels: ["VF 8", "VF 9", "VF 7"],
    cost: 8500000,
    warranty: 36,
    conditions: "Handle with care - fragile",
    stock: 3,
    status: "low_stock",
  },
];

export const mockJobs = [
  {
    id: "1",
    jobNumber: "JOB-001",
    type: "repair",
    vehiclePlate: "30A-12345",
    vehicleModel: "VF 8",
    status: "in_progress",
    assignedStaff: "Tran Thi B",
    assignedTechnician: "Le Van C",
    comment: "Battery module replacement required",
    createdAt: "2024-10-03T09:00:00Z",
    dueDate: "2024-10-05T17:00:00Z",
  },
  {
    id: "2",
    jobNumber: "JOB-002",
    type: "check",
    vehiclePlate: "29B-67890",
    vehicleModel: "VF 9",
    status: "to_do",
    assignedStaff: "Tran Thi B",
    assignedTechnician: "Le Van C",
    comment: "Diagnostic check for infotainment system",
    createdAt: "2024-10-04T08:30:00Z",
  },
];

export const mockRecallCampaigns = [
  {
    id: "1",
    campaignNumber: "RC-2024-001",
    title: "Software Update - Battery Management System",
    description:
      "Critical software update for battery management system to improve charging efficiency",
    affectedModels: ["VF 8", "VF 9"],
    severity: "high",
    status: "active",
    startDate: "2024-09-01T00:00:00Z",
    affectedVehicles: 1250,
    completedVehicles: 890,
  },
  {
    id: "2",
    campaignNumber: "RC-2024-002",
    title: "Brake Pad Replacement - VF 6 Series",
    description: "Preventive replacement of brake pads due to premature wear detected in certain production batches",
    affectedModels: ["VF 6"],
    severity: "medium",
    status: "active",
    startDate: "2024-09-15T00:00:00Z",
    affectedVehicles: 850,
    completedVehicles: 320,
  },
  {
    id: "3",
    campaignNumber: "RC-2024-003",
    title: "Infotainment System Firmware Update",
    description: "Firmware update to fix connectivity issues and improve user interface responsiveness",
    affectedModels: ["VF 7", "VF 8", "VF 9"],
    severity: "low",
    status: "completed",
    startDate: "2024-08-01T00:00:00Z",
    affectedVehicles: 2100,
    completedVehicles: 2100,
  },
  {
    id: "4",
    campaignNumber: "RC-2024-004",
    title: "Steering Column Inspection",
    description: "Mandatory inspection of steering column assembly due to potential manufacturing defect",
    affectedModels: ["VF 5", "VF 6", "VF 7"],
    severity: "high",
    status: "active",
    startDate: "2024-10-01T00:00:00Z",
    affectedVehicles: 1680,
    completedVehicles: 450,
  },
  {
    id: "5",
    campaignNumber: "RC-2024-005",
    title: "Air Conditioning Compressor Replacement",
    description:
      "Replacement of air conditioning compressor units that may fail prematurely in high temperature conditions",
    affectedModels: ["VF 8", "VF 9", "LUX SA2.0"],
    severity: "medium",
    status: "scheduled",
    startDate: "2024-11-01T00:00:00Z",
    affectedVehicles: 950,
    completedVehicles: 0,
  },
  {
    id: "6",
    campaignNumber: "RC-2024-006",
    title: "Headlight Assembly Adjustment",
    description: "Adjustment of headlight assembly alignment to meet updated safety standards",
    affectedModels: ["VF 3", "VF 5"],
    severity: "low",
    status: "completed",
    startDate: "2024-07-15T00:00:00Z",
    affectedVehicles: 1420,
    completedVehicles: 1420,
  },
  {
    id: "7",
    campaignNumber: "RC-2024-007",
    title: "Seat Belt Tensioner Recall",
    description: "Critical recall for seat belt tensioner replacement due to safety concerns",
    affectedModels: ["VF 7", "VF 8"],
    severity: "high",
    status: "active",
    startDate: "2024-09-20T00:00:00Z",
    affectedVehicles: 1150,
    completedVehicles: 780,
  },
  {
    id: "8",
    campaignNumber: "RC-2024-008",
    title: "Tire Pressure Monitoring System Update",
    description: "Software update for TPMS to improve accuracy and reduce false warnings",
    affectedModels: ["VF 6", "VF 7", "VF 8", "VF 9"],
    severity: "low",
    status: "active",
    startDate: "2024-10-05T00:00:00Z",
    affectedVehicles: 2850,
    completedVehicles: 1200,
  },
];

export const mockActivityLogs = [
  {
    id: "1",
    userId: "2",
    userName: "Nguyen Van A",
    action: "Created warranty claim",
    details: "WC-2024-002 for vehicle 29B-67890",
    timestamp: "2024-10-02T10:15:00Z",
    type: "claim",
  },
  {
    id: "2",
    userId: "3",
    userName: "Tran Thi B",
    action: "Updated job status",
    details: "JOB-001 status changed to in_progress",
    timestamp: "2024-10-03T09:05:00Z",
    type: "job",
  },
];

export const vehicleModels = [
  "VF 3",
  "VF 5",
  "VF 6",
  "VF 7",
  "VF 8",
  "VF 9",
  "FADIL",
  "LUX A2.0",
  "LUX SA2.0",
  "Herio Green",
  "Nerio Green",
  "Limo Green",
  "Minio Green",
];
